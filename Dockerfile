...
# install dev tools
RUN apt-get update
RUN apt-get install -y curl tar sudo openssh-server openssh-client rsync openjdk-7-jdk
...
RUN cd /usr/local && tar -xvzf hadoop-2.7.3.tar.gz && ln -s ./hadoop-2.7.3 hadoop
